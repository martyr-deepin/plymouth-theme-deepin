...
{
    connect(m_dbus, &Notification::AllSettingChanged, this, &NotificationWorker::getDbusAllSetting);
    connect(m_dbus, &Notification::appSettingDone, this, &NotificationWorker::getDbusAppsetting);
...
void NotificationWorker::getDbusAllSetting()
{
    QJsonObject obj = QJsonDocument::fromJson(m_dbus->allSetting().toUtf8()).object();
    if (obj.size() > 0) {
...
void NotificationWorker::getDbusSyssetting()
{
    QJsonObject obj = QJsonDocument::fromJson(m_dbus->systemSetting().toUtf8()).object();
    if (obj.size() > 0) {
